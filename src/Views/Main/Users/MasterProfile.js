...
export class MasterProfile extends Component {
  ...
  deleteImage = (id) => {
    ...
    axios
      ...
      .then((res) => {
        createLogs(`Удалил фотографии Мастера ${this.state.master.username}`);
  ...
  deleteAvatar = (userId) => {
    ...
    axios
      ...
      .then((res) => {
        createLogs(`Удалил Аватар Мастера ${this.state.master.username}`);
  ...
  updateMasterType = () => {
    ...
    if (masterType === "COMPANY" && orgName.length < 1) {
      ...
      axios
        ...
        .then((res) => {
          createLogs(`Обновил Тип Мастера ${this.state.master.username}`);
  ...
  pushNotification = () => {
    ...
    if (
      ...
      createLogs(`Отправил Уведомление Мастеру ${this.state.master.username}`);
